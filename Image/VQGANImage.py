...
VQGAN_MODEL_NAMES = ["imagenet", "coco", "wikiart", "sflicker"]
VQGAN_CONFIG_URLS = {
...
class VQGANImage(EMAImage):
  ...
  def __init__(self, width, height, model=VQGAN_MODEL, ema_val=0.99):
    ...
    z = self.rand_latent(vqgan_quantize_embedding = model.quantize.embedding.weight)
    super().__init__(sideX, sideY, z, ema_val)
    self.output_axes = ('n', 's', 'y', 'x')
    self.lr = 0.1

    #extract the parts of VQGAN we need
    self.register_buffer('vqgan_quantize_embedding', model.quantize.embedding.weight) 
  ...
  def init_vqgan(model_name, device = DEVICE):
    ...
    if not path_exists(vqgan_config):
      print(f"WARNING: VQGAN config file {vqgan_cofig} not found. Initializing download.")
    ...
    VQGAN_MODEL = load_vqgan_model(vqgan_config, vqgan_checkpoint).to(DEVICE)
